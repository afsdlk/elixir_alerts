defmodule AlertsWeb.AlertView do
  ...
  def render_repo(repo), do: String.replace(repo, Application.get_env(:alerts, :repo_prefix), "")
  ...
  def render_status(%DB.Alert{status: "under threshold", last_run: date}) do
    Phoenix.HTML.Tag.content_tag(:span, "under threshold#{old(date)}", class: "label label-warning")
  end
